...
export const PROVIDERS: Record<ProviderKey, ProviderConfig> = {
  ...
  vertex: {
    ...
    models: ["imagen-3.0-generate-001", "imagen-3.0-fast-generate-001"].map(id => ({ id, name: formatModelName(id) })),
  },
  openai: {
    displayName: "OpenAI",
...
export const initializeProviderRecord = <T>(defaultValue?: T): Record<ProviderKey, T> =>
  Object.fromEntries(
