...
export async function createCustomerPortalSession(team: Team) { // Using imported Team type
  if (!team.stripeCustomerId || !team.stripeProductId) {
    console.warn(`Stripe customer ID or product ID missing for team ${team.id}. Redirecting to pricing.`);
    redirect('/pricing');
    return;
  }
  ...
  if (configurations.data.length > 0) {
    ...
    if (product.deleted) {
        console.error(`Team's product ${team.stripeProductId} has been deleted in Stripe for team ${team.id}.`);
        throw new Error("Team's product has been deleted in Stripe. Please contact support.");
    }
    if (!product.active) {
      console.error(`Team's product ${team.stripeProductId} is not active in Stripe for team ${team.id}.`);
      throw new Error("Team's product is not active in Stripe. Please contact support.");
    }
    ...
    if (prices.data.length === 0) {
      console.error(`No active prices found for product ${product.id} (team ${team.id}).`);
      throw new Error("No active prices found for the team's product. Please contact support.");
    }
    ...
    configuration = await stripe.billingPortal.configurations.create({
      ...
      features: {
        ...
        customer_update: { enabled: true, allowed_updates: ['email', 'name', 'phone', 'address', 'shipping', 'tax_id'] },
        subscription_update: {
          enabled: true,
...
export async function handleSubscriptionChange(
  ...
  if (!team) {
    console.error('Team not found for Stripe customer ID:', customerId, 'during subscription change.');
    return;
  }
  ...
  if (!planItem || !planItem.product) {
    console.error('Subscription item, plan, or product ID is missing in the subscription object:', subscriptionId);
  ...
  if (typeof productObject === 'string') {
      ...
      console.warn(`Product details for ${productId} (ID only) were not expanded. Product name might be missing or require separate fetch.`);
  } else if (productObject && 'object' in productObject && productObject.id) { // Check if productObject is an object and has an id
      ...
      if (productObject.object === 'product' && !productObject.deleted) {
          ...
          console.warn(`Associated product ${productId} has been deleted in Stripe.`);
      } else if (productObject.object !== 'product') {
           // Product object is not a recognized type or missing 'name'
          console.warn(`Product object for ${productId} is of type ${productObject.object}, not 'product', or 'name' is missing.`);
      }
  } else {
      console.error('Product information is missing or in an unexpected format in subscription item plan.');
  }


  if (!productId) {
    console.error('Could not determine product ID for subscription item:', planItem.id);
    await updateTeamSubscription(team.id, {
          ...
          return {
              ...
              defaultPriceId:
                typeof fullProduct.default_price === 'string'
                  ...
                  : (fullProduct.default_price as Stripe.Price | null)?.id ?? undefined,
          };
    });
  } catch (error) {
...
> => {
  try {
    ...
    return prices.data.map((price) => {
      const productDetails = price.product as Stripe.Product | Stripe.DeletedProduct | null;
      let productName: string | undefined;
      let productDescription: string | null | undefined;
      let productId: string = 'unknown_product'; // Default value

      if (productDetails && 'id' in productDetails) { // Check if productDetails is not null and has an id
        ...
        if (productDetails.object === 'product' && !productDetails.deleted) {
            ...
            productName = 'Deleted Product';
            console.warn(`Price ${price.id} is associated with a deleted product ${productDetails.id}`);
        } else if (productDetails.object !== 'product') {
            console.warn(`Price ${price.id} has an associated product object of type '${productDetails.object}' instead of 'product'.`);
        }
      } else if (typeof price.product === 'string') {
        // Product is not expanded, only ID is available
        productId = price.product;
         console.warn(`Product for price ${price.id} was not expanded. Name/description will be missing.`);
      } else if (price.product === null) {
          console.warn(`Price ${price.id} has a null product field.`);
      } else {
          console.warn(`Price ${price.id} is missing product information or product ID in an unexpected format.`);
      }
