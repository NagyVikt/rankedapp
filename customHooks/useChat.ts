...
const useChat = () => {
    ...
    const initializeChat = async () => {
      try {
        ...
        const history = messages.map((msg) => ({
          // Convert local messages to library format if needed
          role: msg.role,
        ...
        chatSessionRef.current = model.startChat({
          generationConfig: {
            // Pass the generation config subset valid for startChat
        ...
        setError(
          err instanceof Error
            ? err
            : new Error('Unknown chat initialization error'),
        );
      }
    };
  ...
  const sendMessage = async (userMessage: string) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error('Failed to get response'),
      );
