...
const filterModelConfig = (
  ...
  return Object.fromEntries(
    Object.entries(config).filter(([key]) =>
      ALLOWED_PROVIDER_KEYS.includes(key as AllowedProviderKey),
    ),
...
const filterEnabledProviders = (
  ...
  console.log(
    'DEBUG: Record after filtering in filterEnabledProviders:',
    filtered,
  );
  return filtered;
};
...
export function ImagePlayground({
  ...
  const [enabledProviders, setEnabledProviders] =
    useState<FilteredEnabledRecord>(() => {
      const allProvidersRecord = initializeProviderRecord(true);
      console.log(
        'DEBUG: Output of initializeProviderRecord(true):',
        allProvidersRecord,
      );
      const initialEnabled = filterEnabledProviders(allProvidersRecord);
      console.log(
        'DEBUG: Initializing enabledProviders state:',
        initialEnabled,
      );
  ...
  const handlePromptSubmit = (newPrompt: string) => {
    ...
    console.log(
      'DEBUG: Submitting prompt. Active providers:',
      activeProviders,
      'Models:',
      providerToModel,
    );
  ...
  const calculatedModelProps = ALLOWED_PROVIDER_KEYS.map((key) => {
    ...
    console.log(
      `DEBUG: [Render] Provider config for ${key}:`,
    ...
    if (!provider) {
      console.warn(
        `DEBUG: [Render] Provider configuration missing in PROVIDERS for key: ${key}. Skipping.`,
      );
      return null;
    }
    ...
    if (
      ...
      !provider.models ||
      provider.models.length === 0 || // provider.models is Model[]
      !provider.iconPath ||
      !provider.color
    ) {
      console.warn(
        `DEBUG: [Render] Provider configuration for key: ${key} is incomplete (missing displayName, models, iconPath, or color). Skipping.`,
      );
      return null;
    }
    ...
    if (typeof currentSelectedModelValue === 'undefined') {
      console.warn(
        `DEBUG: [Render] No model selected for provider ${key} (selectedModels['${key}'] is undefined). Skipping card for this provider as 'value' prop would be undefined.`,
      );
      return null;
    }
    ...
    const modelIdsForSelect: string[] = provider.models.map(
  ...
  console.log(
    `DEBUG: [Render] Number of valid model props calculated: ${calculatedModelProps.length}`,
  );
  console.log(
    'DEBUG: [Render] Final props for rendering:',
    calculatedModelProps,
  );
        ...
        <>
          {numProviders > 0 ? (
            <>
              ...
              <div
                className={`hidden md:grid ${gridColsClass} ${xlGridColsClass} gap-8`}
              >
