  ...
  return (
    ...
    >
      ...
      <NavbarContent className="basis-1/5 sm:basis-full" justify="start">
        ...
        <NavbarBrand as={Link} href="/" className="flex items-center gap-2 text-foreground"> {/* Ensure brand uses foreground color */}
          <Logo className="text-foreground" /> {/* Assuming Logo can take a className to control color, or it inherits */}
          <p className="font-bold text-inherit">RANKED</p> {/* Inherits text-foreground from NavbarBrand */}
        </NavbarBrand>
        ...
        <div className="hidden lg:flex gap-2 justify-start ml-4">
          {siteConfig.navItems.map((item: SiteNavItem) => (
            <NavbarItem key={item.href} isActive={pathname === item.href}>
              ...
              >
                {item.icon && <Icon icon={item.icon} width={20} height={20} />} {/* Icon size adjusted slightly */}
                {item.label}
              </Link>
            </NavbarItem>
          ))}
      ...
      {/* Navbar Content: End */}
      <NavbarContent className="hidden sm:flex basis-1/5 sm:basis-full items-center" justify="end">
        <NavbarItem>
          <ThemeSwitch />
        </NavbarItem>
