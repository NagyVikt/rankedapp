...
const NotificationsSettingsSkeleton = ({
  ...
  <div className={`space-y-3 max-w-lg p-2 ${className}`}>
    {' '}
    {/* Added max-w and padding */}
    {/* Header Skeleton */}
    ...
    <div className="flex flex-col gap-2 px-4">
      {[...Array(7)].map(
        (
          _,
          i, // Simulate 7 switch rows
        ) => (
          <div
            key={i}
            className="flex justify-between items-center py-2 min-h-[56px]"
          >
            {' '}
            {/* Mimic SwitchCell structure + min-height */}
            ...
            <SimpleSkeleton className="h-6 w-11 rounded-full" />{' '}
            {/* Switch Skeleton */}
          </div>
        ),
      )}
...
const SwitchCell: React.FC<SwitchCellProps> = ({
  ...
  return (
    <div
      className={`flex items-center justify-between gap-2 rounded-medium p-3 ${className}`}
    >
...
export default function NotificationsSettings({
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        const fetchedSettings: NotificationPreferences = {
          // Example data
          pauseAll: false,
          followers: true,
          likes: true,
  ...
  if (error) {
    return (
      <div
        className={`w-full max-w-lg p-4 text-center text-danger ${className}`}
      >
        Error loading notification settings: {error.message}. Please try
        refreshing.
      </div>
    );
  }
  ...
  if (!settings) {
    return (
      <div
        className={`w-full max-w-lg p-4 text-center text-default-500 ${className}`}
      >
      ...
      <div className="p-4">
        ...
        <form className="flex flex-col gap-2" onSubmit={handleSubmit}>
          ...
          <SwitchCell
            ...
            onValueChange={(value) =>
              handleSettingChange('friendRequests', value)
            }
            name="friendRequests"
          />
