...
export default function ShopifyTab({ className }: ShopifyTabProps) {
  ...
  const handleSaveConnection = () => {
    const trimmedDomain = shopDomain
      .trim()
      .toLowerCase()
      .replace(/^https?:\/\//, '')
      .split('/')[0];
    ...
    if (!trimmedDomain || !trimmedToken) {
      alert(
        'Please provide both the Shop Domain (e.g., your-store.myshopify.com) and the Admin API Access Token.',
      );
      return;
    }
    ...
    const newEntry: ShopifyEntry = {
      shopDomain: trimmedDomain,
      accessToken: trimmedToken,
    };
    let updatedConnections: ShopifyEntry[];
    if (isEditing && editingIndex !== null) {
      const conflictExists = connections.some(
        (conn, index) =>
          index !== editingIndex && conn.shopDomain === trimmedDomain,
      );
      if (conflictExists) {
      ...
      const exists = connections.some(
        (conn) => conn.shopDomain === trimmedDomain,
      );
      if (exists) {
        alert(`A connection for ${trimmedDomain} already exists.`);
        return;
    ...
    <div className={`space-y-4 pt-4 ${className}`}>
      ...
      {connections.length === 0 ? (
        <p className="text-center text-default-500 py-4">
          No Shopify connections configured yet.
        </p>
      ) : (
            ...
            <>
              ...
              <ModalBody className="space-y-4">
                <p className="text-sm text-foreground-500">
                  You can create a Custom App in your Shopify Admin under 'Apps
                  and sales channels' &gt; 'Develop apps' to get an Admin API
                  access token. Ensure it has the necessary permissions (e.g.,
