...
async function setAccountPinAction(
  formData: FormData,
): Promise<{ success?: string; error?: string }> {
...
async function togglePinRequirementAction(
  isEnabled: boolean,
): Promise<{ success?: string; error?: string }> {
...
const fetcher = async (url: string): Promise<User> => {
  ...
  if (
    typeof data?.pinEnabled === 'undefined' ||
    typeof data?.isPinSet === 'undefined'
  ) {
...
const SecuritySettingsSkeleton = ({ className }: { className?: string }) => (
  <div className={`space-y-2 max-w-4xl ${className}`}>
    {' '}
    ...
    <div className="px-0">
      {' '}
      {/* Remove padding here, CellWrapper might have it */}
      {[...Array(7)].map(
        (
          _,
          i, // Simulate 7 setting rows (incl. delete)
        ) => (
          <div
            key={i}
            className="flex justify-between items-center p-3 border-b border-gray-100 min-h-[68px]"
          >
            {' '}
            {/* Mimic CellWrapper structure */}
            <div className="flex-grow mr-4 space-y-1.5">
              <SimpleSkeleton className="h-4 w-32 rounded" /> {/* Label */}
              <SimpleSkeleton className="h-3 w-48 rounded" />{' '}
              {/* Description */}
            </div>
            {/* Make last button red-ish */}
            <SimpleSkeleton
              className={`h-8 w-28 rounded-full ${i === 6 ? 'bg-red-200' : ''}`}
            />{' '}
            {/* Button Skeleton */}
          </div>
        ),
        ...
        <div className="space-y-2 p-4">
            ...
            <div className="flex flex-wrap items-center justify-end gap-4 sm:flex-nowrap">
              <div className="flex flex-col items-end">
                ...
                {user.emailVerified === true && (
                  <p className="text-tiny text-success">Verified</p>
                )}
                {user.emailVerified === false && (
                  <p className="text-tiny text-warning">Not Verified</p>
                )}
          ...
          <CellWrapper>
            ...
            <div className="flex items-center gap-2">
              <p className="text-small text-default-400 mr-2">Not Enabled</p>{' '}
              {/* Example status */}
              <Button size="sm" radius="full" variant="bordered" isDisabled>
                Setup 2FA
              </Button>
            </div>
          </CellWrapper>
          ...
          <form onSubmit={handleAccountSubmit}>
            ...
            <ModalBody>
              <p className="text-small text-default-500 mb-4">
                {' '}
                Update your name and email address. Changing your email might
            ...
            <ModalFooter>
              ...
              <Button
                color="primary"
                type="submit"
                isLoading={accountState.pending}
                startContent={
                  !accountState.pending ? (
                    <Icon icon="solar:diskette-linear" />
                  ) : null
                }
                isDisabled={accountState.pending}
              >
                {' '}
                {accountState.pending ? 'Saving...' : 'Save Changes'}{' '}
              </Button>
            </ModalFooter>
          </form>
        </ModalContent>
      ...
      >
        <ModalContent>
          <form onSubmit={handlePasswordSubmit}>
            ...
            <ModalBody>
              <p className="text-small text-default-500 mb-4">
                Enter your current password and set a new one. Use at least 8
                characters.
              </p>
              <div className="space-y-4">
                <div>
            ...
            <ModalFooter>
              ...
              <Button
                ...
                isDisabled={passwordState.pending}
              >
                {' '}
          ...
          <form onSubmit={handleDeleteSubmit}>
            ...
            <ModalBody>
              <p className="text-danger font-medium mb-4">
                ...
                will be irreversibly deleted. Please confirm by entering your
                password.{' '}
              </p>
              <div className="space-y-4">
                <div>
                  <Input
                    ...
                    endContent={
                      <PasswordToggleIcon
                        isVisible={showDeleteConfirmPassword}
                        onPress={() =>
            ...
            <ModalFooter>
              ...
              <Button
                color="danger"
                type="submit"
                isLoading={deleteState.pending}
                startContent={
                  !deleteState.pending ? (
                    <Icon icon="solar:trash-bin-trash-linear" />
                  ) : null
                }
                isDisabled={deleteState.pending}
              >
                {' '}
                {deleteState.pending
                  ? 'Deleting...'
