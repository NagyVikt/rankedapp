...
const RELEVANT_API_KEYS = [
  'openaiApiKey', 'geminiApiKey', 'claudeApiKey', 'perplexityApiKey', 'deepseekApiKey'
];
...
export default function SettingsComponent(props: CardProps): JSX.Element {
  ...
  const getInitialTabKey = useCallback((): TabKey => {
    ...
    if (stored && ['account','notifications','security','woocommerce','shopify','apikeys','plugin-settings'].includes(stored)) {
      return stored;
    }
    return 'account';
  }, []);
  ...
  const handleSelectionChange = (key: string) => {
    ...
    if (typeof window !== 'undefined') localStorage.setItem(SELECTED_TAB_STORAGE_KEY, tab);
  };

  const renderIndicator = (count?: number, configured?: boolean): React.ReactNode => {
    const base = "ml-1 static right-auto transform-none";
    if (count !== undefined) {
      if (count > 0) {
        return (<ChipX size="sm" color="success" variant="flat" className={base}>{count}</ChipX>) as unknown as React.ReactNode;
      } else {
        return (<BadgeX color="default" variant="dot" isInvisible={false} className={base} />) as unknown as React.ReactNode;
      }
    }
    return (<BadgeX color={configured ? "success" : "default"} variant="dot" isInvisible={false} className={base} />) as unknown as React.ReactNode;
  };
  ...
  return (
    <CardX {...props} className="overflow-hidden bg-background">
      {!mounted ? (
        ...
        >
          <TabX value="account" title={renderTabTitle('solar:user-circle-linear','Account')}>
            <AccountDetailsX />
          </TabX>
          <TabX value="notifications" title={renderTabTitle('solar:bell-outline','Notifications')}>
            <NotificationsSettingsX />
          </TabX>
          <TabX value="security" title={renderTabTitle('solar:shield-check-linear','Security')}>
            <SecuritySettingsX />
          </TabX>
          <TabX value="woocommerce" title={renderTabTitle('logos:woocommerce','WooCommerce', renderIndicator(wooCount))}>
            <WooCommerceTabX />
          </TabX>
          <TabX value="shopify" title={renderTabTitle('logos:shopify','Shopify', renderIndicator(shopifyCount))}>
            <ShopifyTabX />
          </TabX>
          <TabX value="apikeys" title={renderTabTitle('material-symbols:key-outline','API Keys', renderIndicator(undefined, apiKeysConfiguredCount>0))}>
            <ApiKeysTabX />
          </TabX>
          <TabX value="plugin-settings" title={renderTabTitle('material-symbols:settings-outline','Plugin settings', renderIndicator(undefined, apiKeysConfiguredCount>0))}>
            <ApiKeysTabX />
          </TabX>
        </TabsX>
      )}
