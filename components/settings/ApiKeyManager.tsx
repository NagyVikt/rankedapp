...
export function ApiKeyManager({ config }: ApiKeyManagerProps) {
  ...
  const saveKey = useCallback(
    (keyToSave: string) => {
      ...
      if (config.keyPrefix && !trimmedKey.startsWith(config.keyPrefix)) {
        alert(
          `Invalid ${config.name} API Key format. It should start with '${config.keyPrefix}'.`,
        );
        return false;
      }
      ...
      try {
        ...
        alert(
          `Failed to save ${config.name} API key. Storage might be full or disabled.`,
        );
  ...
  const handleRemoveClick = () => {
    if (
      window.confirm(
        `Are you sure you want to remove the ${config.name} API Key?`,
      )
    ) {
      removeKey();
    }
    ...
    <div className="p-4 bg-content1 border border-divider rounded-lg flex flex-wrap items-center justify-between gap-4 min-h-[76px]">
        ...
        <div>
          ...
          {isConfigured ? (
            <p className="text-small text-foreground-500">
              API Key:{' '}
              <span className="font-mono bg-content2 px-1 rounded text-xs">
                {maskApiKey(apiKey)}
              </span>{' '}
              {/* Use content2 for nested bg */}
            </p>
          ) : (
      ...
      >
        <ModalContent>
          {(modalOnClose) => (
            <>
              <ModalHeaderX className="flex flex-col gap-1 text-foreground">
                {' '}
                {/* Ensure header text color */}
                {isConfigured
                  ? `Edit ${config.name} API Key`
                  : `Configure ${config.name} API Key`}
              </ModalHeaderX>
              <ModalBodyX>
                <p className="text-small text-foreground-500 mb-2">
                  {' '}
                  {/* Body text color */}
                  Enter your secret API key from {config.name}. It's stored only
                  in your browser.
                </p>
                {config.infoText && (
                  <p className="text-xs text-warning mb-3">{config.infoText}</p>
                )}
                <InputX
