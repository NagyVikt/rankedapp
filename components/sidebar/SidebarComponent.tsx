...
import {
  ...
  DropdownTrigger,
  DropdownMenu,
  DropdownItem, // Added Dropdown components
} from '@heroui/react'; // Ensure this path is correct and all components are exported
import { Icon } from '@iconify/react';
import useSWR from 'swr';
import {
  navigationSections,
  SidebarItem as SidebarItemProps,
  SidebarSection,
  SidebarItemType,
} from './sidebar-items'; // Ensure this path is correct
...
export default function SidebarComponent() {
  ...
  const { data: userData, error: userError } = useSWR<{ user: User }>(
    '/api/user',
    fetcher,
  );
  ...
  const [selectedLanguageKey, setSelectedLanguageKey] =
    useState<React.Key>('en');
  const selectedLanguage = React.useMemo(
    () =>
      languageOptions.find((lang) => lang.key === selectedLanguageKey) ||
      languageOptions[0],
    [selectedLanguageKey],
  );
  ...
  useEffect(() => {
    if (user && typeof window !== 'undefined') {
      // Ensure window is defined for origin
      // Generate referral link once user data is available
      setReferralLink(
        `${window.location.origin}/referral/${user.id || user.email.split('@')[0]}`,
      );
    }
  }, [user]);
  ...
  >(
    navigationSections.reduce(
      (acc, section) => {
        if (section.title) {
          // Only sections with titles are collapsible
  ...
  const [isEmailStatsVisible, setIsEmailStatsVisible] = useState(
    ...
    navigationSections
      .flatMap((s) => s.items)
      .find((i) => i.type === SidebarItemType.EmailStatsToggle)
  ...
  const NavItemSkeleton = ({ isSubItem = false }: { isSubItem?: boolean }) => (
    <div
      className={`flex items-center gap-3 h-11 ${isSubItem ? 'px-3 pl-10' : 'px-3'}`}
    >
      <Skeleton className="w-6 h-6 rounded-md" />
      <Skeleton
        className={`w-3/4 h-4 rounded-md ${isSubItem ? 'w-1/2' : ''}`}
      />
    </div>
  );
  ...
  const SectionTitleSkeleton = () => (
    <div className="px-3 py-3 mt-2">
      {' '}
      {/* Increased py for better spacing */}
  ...
  const renderSidebarItem = (
    ...
    if (
      item.type === SidebarItemType.EmailStatsToggle &&
      typeof item.endContent === 'function'
    ) {
      return (
        <div key={`${item.key}-${index}`} className="mt-1 mb-0.5 px-1">
          {' '}
          {/* Added px-1 for slight indent */}
          {item.endContent({
            isItemOpen: isEmailStatsVisible,
            onToggle: toggleEmailStatsVisibility,
          })}
        </div>
      );
    }
    ...
    if (item.isStatistic) {
      ...
      return (
        <div
          key={`${item.key}-${index}`}
          className={`flex justify-between items-center ${item.className || 'text-sm text-default-600 px-3 py-0.5'} ${isSubItem ? 'pl-10' : 'pl-4'}`}
        >
          {' '}
          {/* Consistent padding */}
          ...
          {item.statisticValue && (
            <span className="font-medium text-default-700 dark:text-default-300 text-xs">
              {item.statisticValue}
            </span>
          )}
        </div>
    ...
    if (isActive && !isSubItem) {
      if (
        item.className &&
        (item.className.includes('bg-') || item.className.includes('text-'))
      ) {
    ...
    if (item.key === 'webshops' && typeof item.endContent === 'function') {
      ...
      item.type === SidebarItemType.Dropdown &&
      typeof item.endContent === 'function'
    ) {
      actualEndContent = item.endContent({
        isItemOpen: openDropdownItems[item.key],
      });
    }
    ...
    const itemContent = (
      <Button
        as={
          item.href && item.type !== SidebarItemType.Dropdown ? Link : 'button'
        }
        href={item.href}
        onClick={
          item.type === SidebarItemType.Dropdown
            ? () => toggleDropdownItem(item.key)
            : item.onClick
        }
        fullWidth
        variant="light"
        color={
          isActive &&
          !(item.className && item.className.includes('bg-')) &&
          !isSubItem
            ? 'primary'
            : 'default'
        }
        ...
        startContent={
          item.icon && (
            <Icon
              icon={item.icon}
              width={isSubItem ? 18 : 22}
              height={isSubItem ? 18 : 22}
              className={isSubItem ? 'text-default-500 mr-1' : ''}
            />
          )
        }
        endContent={actualEndContent as React.ReactNode}
      >
        <span className={`flex-grow text-left ${isSubItem ? 'text-xs' : ''}`}>
          {item.title}
        </span>
      </Button>
    );
  ...
  return (
    <>
      ...
      */}
      <div
        className={`flex h-full flex-col p-4 relative border-r border-divider bg-background ${GLOBAL_NAVBAR_HEIGHT_CLASS}`}
      >
        {/* Header: Brand and Toggle Button */}
        <div className="flex items-center justify-between px-2 mb-1">
          <div className="flex items-center gap-2">
            <Skeleton
              isLoaded={!isSidebarLoading}
        ...
        <div className="px-2 space-y-3">
          {' '}
          {/* Added space-y-3 for separation between profile and referral */}
          <div className="flex items-start gap-3">
            {' '}
            {/* Changed to items-start for better alignment with dropdown */}
            <Skeleton
              isLoaded={!isLoadingUser}
              className="rounded-full w-12 h-12 text-lg flex-shrink-0"
            >
              <Avatar
                isBordered
                size="lg"
            ...
            <div className="flex flex-col justify-center flex-grow overflow-hidden">
              <Skeleton
                isLoaded={!isLoadingUser}
                className="h-5 w-4/5 rounded-lg mb-1.5"
              >
                {user && (
              ...
              <Skeleton
                isLoaded={!isLoadingUser}
                className="h-4 w-3/5 rounded-lg"
              >
                {user && (
            ...
            <Skeleton
              isLoaded={!isLoadingUser}
              className="w-12 h-8 rounded-md ml-auto flex-shrink-0"
            >
              {user && (
                <Dropdown placement="bottom-end">
                  <DropdownTrigger>
                    <Button
                      isIconOnly
                      variant="light"
                      className="w-10 h-10 min-w-0 data-[hover=true]:bg-default-100 dark:data-[hover=true]:bg-default-50"
                    >
                      <Icon
          ...
          <Skeleton isLoaded={!isLoadingUser} className="h-20 rounded-lg">
            {user && referralLink && (
              <div className="p-3 bg-default-100 dark:bg-default-50 rounded-lg shadow-sm border border-default-200 dark:border-default-700">
                <p className="text-xs font-medium text-default-700 dark:text-default-300 mb-1.5">
                  Your Referral Link:
                </p>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    classNames={{
                      inputWrapper:
                        'h-9 text-xs bg-background dark:bg-default-100 border-default-300 dark:border-default-200 focus-within:border-primary dark:focus-within:border-primary',
                      input:
                        'text-default-700 dark:text-default-200 placeholder:text-default-400',
                    }}
        ...
        <ScrollShadow className="flex-grow -mr-4 pr-4 py-2" hideScrollBar>
          {isSidebarLoading ? (
            <nav className="flex flex-col gap-0.5">
              {[...Array(3)].map((_, i) => (
                <NavItemSkeleton key={`navskel-${i}`} />
              ))}
              <SectionTitleSkeleton />
              {[...Array(4)].map((_, i) => (
                <NavItemSkeleton key={`navskel2-${i}`} />
              ))}
              <NavItemSkeleton isSubItem={true} />
              <SectionTitleSkeleton />
              {[...Array(2)].map((_, i) => (
                <NavItemSkeleton key={`navskel3-${i}`} />
              ))}
            </nav>
          ) : (
            <nav className="flex flex-col gap-0.5">
          ...
          {isPluginCardVisible &&
            (isSidebarLoading ? (
              <Skeleton className="mx-2 h-40 rounded-lg mb-10" />
            ) : (
              <Card
                className="mx-2 overflow-visible mb-10 relative"
                shadow="sm"
                isBlurred
              >
                <Button
                  ...
                  aria-label="Hide plugin card"
                  onPress={handleHidePluginCard}
                >
        ...
        {isSidebarLoading ? (
          ...
          <div className="flex flex-col gap-1 pt-2 border-t border-divider">
            <Button
              fullWidth
              radius="md"
              className="justify-start text-default-600 text-sm h-10"
              startContent={
            ...
            <Button
              ...
              color="danger"
              startContent={
                <Icon icon="solar:logout-3-line-duotone" width={20} />
              }
              variant="light"
