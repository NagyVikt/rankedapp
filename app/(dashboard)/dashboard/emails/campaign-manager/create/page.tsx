...
interface DesignItem {
  ...
  interface EditingGroup { // This is the type for the 'editingGroup' state in the original canvas context
    isDefault: boolean;
    id: string;
            ...
            <>
              ...
              <ModalBody className="py-4 space-y-4">
                <Switch
                  ...
                  size="md"
                  color="primary"
                  isDisabled={editingGroup?.isDefault && editingGroup.id === 'group-send-now'}
                >
                ...
                {!newGroupSupportsIndividual && (
                    ...
                    selectedKey={ newGroupDesignId !== null ? String(newGroupDesignId) : null }
                    ...
                    >
                    {item => (
                        <Item key={String(item.id)}>{item.name}</Item>
                    )}
                    </Picker>
              ...
              <ModalFooter className="border-t border-gray-200 dark:border-neutral-700">
                <Button color="danger" variant="light" onPress={onClose} className="rounded-md">
                  Cancel
                </Button>
                <Button
                  color="primary"
...
function CampaignEditor() {
  ...
  const [editingGroupModal, setEditingGroupModal] = useState<UserGroup | null>(null); // Renamed to avoid conflict with 'EditingGroup' type if it were a state var name
   ...
   useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (sendNowGroup && mockCustomers.length > customerIdx) {
        const num = Math.min(mockCustomers.length - customerIdx, Math.floor(Math.random() * 3) + 1);
        sendNowGroup.emails = mockCustomers.slice(customerIdx, customerIdx + num) as string[];
        customerIdx += num;
        if (
          designs.length > 0 &&
      ...
      if (abandonedGroup && mockCustomers.length > customerIdx) {
        const num = Math.min(mockCustomers.length - customerIdx, Math.floor(Math.random() * 4) + 2);
        abandonedGroup.emails = mockCustomers.slice(customerIdx, customerIdx + num);
  ...
  const handleOpenGroupModalInEditor = (group?: UserGroup) => { // Renamed to avoid conflict
    setEditingGroupModal(group || null);
    setNewGroupName(group?.name || '');
    setNewGroupDescription(group?.description || '');
    setNewGroupSupportsIndividual(group ? group.supportsIndividualDesignAssignment : false);
  ...
  const handleSaveGroupInEditor = () => { // Renamed to avoid conflict
    if (!newGroupName.trim()) {
        showNotification("Group name cannot be empty.", "error");
        return;
    }
    if (editingGroupModal) {
        setUserGroups(prev => prev.map(g => g.id === editingGroupModal.id ? {
            ...g, 
  ...
  const handleDeleteGroupInEditor = (groupId: string) => { // Renamed to avoid conflict
                                ...
                                <div className="w-full xl:w-[380px] xl:flex-shrink-0 order-1">
                                    ...
                                    <Card className={`p-0 flex flex-col min-h-[400px] max-h-[calc(100vh-350px)] border rounded-lg shadow-sm border-gray-300 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800/50`}>
                                        <CardHeader className="sticky top-0 z-10 bg-gray-50 dark:bg-neutral-800/50 flex flex-col items-start p-3 sm:p-4 border-b border-gray-200 dark:border-neutral-700 space-y-2">
                                            ...
                                            <Input isClearable fullWidth size="sm" placeholder="Search all customers..." value={poolSearchTerm} onValueChange={setPoolSearchTerm} className="bg-white dark:bg-neutral-700 rounded-md"
                                                    ...
                                                    {showBulkMoveFromPoolOptions && (
                                                       ...
                                                       <Select
                                                            ...
                                                            onSelectionChange={(keys) =>
                                                                ...
                                                                Array.from(keys as Set<React.Key>)[0] as string
                                                                )
                                                            }
                                                            classNames={{
                                                                trigger: "bg-white dark:bg-neutral-600/50 rounded-md text-xs",
                                                                popoverContent: "bg-white dark:bg-neutral-800 border rounded-md",
                                                            }}
                                                            >
                                                            {userGroups.map(ug => (
                                                    ...
                                                    <CardHeader className={`sticky top-0 z-10 flex flex-col items-start p-3 sm:p-4 border-b border-gray-200/50 dark:border-neutral-600/50 space-y-2 ${group.colorScheme.light} ${group.colorScheme.dark}`}>
                                                        <div className="w-full flex justify-between items-center">
                                                            ...
                                                            <DropdownMenu
                                                                ...
                                                                onAction={(key) => {
                                                                    if (key === 'edit') handleOpenGroupModalInEditor(group);
                                                                    else if (key === 'delete') handleDeleteGroupInEditor(group.id);
                                                                }}
                                                                >
                                                                {/* always show “Edit” */}
                                                        ...
                                                        {paginatedGroupUserEmails.length > 0 && (
                                                            <div className="w-full flex flex-col sm:flex-row items-start sm:items-center justify-between mt-1 gap-2">
                                                                ...
                                                                <Button size="sm" color="danger" variant="flat" onPress={() => handleBulkRemoveFromGroup(group.id)} isDisabled={selectedInThisGroup.size === 0} className="rounded-md">
                                                      ...
                                                      {group.supportsIndividualDesignAssignment &&
                                                        ...
                                                        <div className="w-full mt-1 text-xs">
                                                            ...
                                                            onSelectionChange={(key) => {
                                                                ...
                                                                const id = key != null && key !== '' ? Number(key) : null
                                                                const design = designs.find(d => d.id === id)
                                                                if (design) {
                                                                handleBulkAssignDesignToSelectedEmails(group.id, design)
                                                                }
                                                            }}
                                ...
                                <Card ref={provided.innerRef} {...provided.droppableProps} className={`p-3 min-h-[80px] border rounded-lg shadow-sm bg-gray-100 dark:bg-neutral-800/70 border-gray-200 dark:border-neutral-700 ${snapshot.isDraggingOver ? 'bg-blue-50 dark:bg-blue-900/30' : ''}`}>
                                    <CardBody className="flex flex-row gap-3 items-center overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-neutral-600 scrollbar-track-transparent pb-2">
                                        {designs.length > 0 ? designs.map((design, index) => (
                                            <Draggable key={design.id.toString()} draggableId={`design-${design.id}`} index={index}>
                                                {(dp, ds) => (
                        ...
                        <ModalBody className="py-4">
                            <div className="space-y-4 mt-4">
                                ...
                                <Select
                                    ...
                                    >
                                    <SelectItem key="badgeApplied" className="text-black dark:text-neutral-200">
                                        Badge Applied
                                    </SelectItem>
                                    <SelectItem key="specificDate" className="text-black dark:text-neutral-200">
                                        Specific Date
                                    </SelectItem>
                                    <SelectItem key="recurring" className="text-black dark:text-neutral-200">
                                        Recurring
                                    </SelectItem>
                                    <SelectItem key="manualAdd" className="text-black dark:text-neutral-200">
                        ...
                        <ModalFooter className="border-t border-gray-200 dark:border-neutral-700">
                            ...
                            <Button color="primary" onPress={() => {
                                ...
                                const mockAutoData: Automation = {
                                    ...
                                    trigger: editingAutomation?.trigger || {type: 'manualAdd'}, // Placeholder
    ...
    >
      <ModalContent className="bg-white dark:bg-neutral-800 text-black dark:text-neutral-100 border border-gray-300 dark:border-neutral-700 rounded-lg">
        {(onCloseModal) => (
          <>
            <ModalHeader className="border-b border-gray-200 dark:border-neutral-700 text-lg font-semibold">
              {editingGroupModal ? 'Edit Email Group' : 'Create New Email Group'}
            </ModalHeader>
            <ModalBody className="py-4 space-y-4">
              {/* Form for Group Name */}
              <Input
              ...
              <div className="space-y-1">
                  ...
                  <div className="flex gap-2 flex-wrap">
                      {DEFAULT_GROUP_COLORS.map(color => (
                          <Button key={color.light} isIconOnly size="sm"
