...
const initialAIJobs: AIJob[] = [
  // ... (Full list of jobs as defined previously) ...
  {
    ...
    description:
      'Sends personalized recovery emails/SMS for abandoned checkouts.',
    icon: 'solar:cart-large-minimalistic-broken',
    isEnabled: true,
    status: 'Idle',
    lastRun: '2025-05-03 10:00',
    badge: 'HOT',
    configAction: {
      label: 'Configure Sequence',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    id: 'welcome-email',
    title: 'Personalized Welcome Agent',
    description:
      'Drafts & sends personalized welcome emails to new customers/subscribers.',
    icon: 'solar:letter-opened-broken',
    isEnabled: true,
    status: 'Completed',
    lastRun: '2025-05-04 09:15',
    configAction: {
      label: 'Edit Template',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
  ...
  },
  {
    ...
    description:
      'Writes compelling, SEO-friendly descriptions for new or existing products.',
    icon: 'solar:document-add-broken',
    isEnabled: true,
    status: 'Idle',
    badge: 'NEW',
    configAction: {
      label: 'Configure Tone & Keywords',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    id: 'ad-copy-gen',
    title: 'Ad Generator Agent',
    description:
      'Creates variations of ad copy (Google, Facebook etc.) based on product details.',
    icon: 'solar:pen-new-square-linear',
    isEnabled: false,
    status: 'Idle',
    badge: 'NEW',
    configAction: {
      label: 'Set Audience & Goals',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    ...
    description:
      'Analyzes content & suggests SEO keyword improvements. Generates meta descriptions.',
    icon: 'solar:magnifer-zoom-in-linear',
    isEnabled: false,
    status: 'Idle',
    configAction: {
      label: 'Analyze Page / Set Keywords',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    id: 'product-tagger',
    title: 'Automated Product Tagger Agent',
    description:
      'Suggests relevant tags & categories for products based on title/description/image.',
  ...
  },
  {
    ...
    title: 'AI Image Tool',
    description:
      'Automatically removes or replaces backgrounds from product photos.',
  ...
  },
  {
    ...
    title: 'Inventory Forecaster Agent',
    description:
      'Predicts future stock needs based on sales data, seasonality, and trends.',
    icon: 'solar:chart-line-broken',
    isEnabled: false,
    status: 'Idle',
    configAction: {
      label: 'Connect Data / View Forecast',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    id: 'churn-predictor',
    title: 'Customer Churn Predictor AI',
    description:
      'Identifies customers at high risk of churning based on behavior patterns.',
    icon: 'solar:user-minus-broken',
    isEnabled: false,
    status: 'Idle',
    configAction: {
      label: 'View At-Risk Customers',
      onPress: (id) => alert(`Configure ${id}`),
      icon: 'solar:settings-linear',
    },
  },
  {
    ...
    description:
      'Identifies potential social media influencers based on custom criteria.',
  ...
  },
  {
    ...
    title: 'Dynamic Pricing Assistant AI',
    description:
      'Suggests optimal price adjustments based on demand, competition, and inventory.',
...
const badgeColorMap: Record<
  Required<AIJob>['badge'],
  'secondary' | 'danger' | 'warning' | 'default'
> = {
  NEW: 'secondary',
  HOT: 'danger',
  TRENDING: 'warning',
...
const useFetchJobs = (initialData: AIJob[]) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (typeof window !== 'undefined') {
        ...
        const enabledMap: Record<string, boolean> = storedState
          ? JSON.parse(storedState)
          : {};
        const mergedData = initialData.map((job) => ({
          ...job,
          isEnabled:
            enabledMap[job.id] !== undefined
              ? enabledMap[job.id]
              : job.isEnabled,
        }));
        setJobs(mergedData);
  ...
  const updateAndPersistJobs = useCallback(
    (updater: React.SetStateAction<AIJob[]>) => {
      setJobs((currentJobs) => {
        ...
        if (typeof window !== 'undefined') {
          const enabledMap = newJobs.reduce(
            (acc, job) => {
...
export default function AIJobTrackerComponent() {
  ...
  const JobCardSkeleton = () => (
    <Card
      shadow="sm"
      className="flex flex-col bg-content1 border border-divider min-h-[180px]"
    >
      {' '}
      {/* Added min-height */}
          ...
          : aiJobs.map((job) => {
              ...
              return (
                ...
                >
                  <Card
                    shadow="sm"
                    className="flex flex-col bg-content1 border border-divider"
                  >
                    <CardHeader className="flex justify-between items-start gap-3 p-4">
                      <div className="flex items-center gap-3">
                        <Icon
                          ...
                          width={28}
                          className={
                            disabled ? 'text-default-400' : 'text-primary'
                          }
                        />
                      ...
                      >
                        <div className={disabled ? 'pointer-events-auto' : ''}>
                          <Switch
                            ...
                            onValueChange={(sel) =>
                              handleToggleEnable(job.id, sel)
                            }
                            size="sm"
                            classNames={{
                              wrapper: [
                                'group-data-[selected=true]:bg-orange-600',
                              ],
                            }}
                          />
                        </div>
                    ...
                    <CardFooter className="flex justify-between items-center p-4 pt-2">
                      ...
                      <div className="flex items-center gap-1">
                        {job.configAction && (
                          <Tooltip
                            content={job.configAction.label}
                            placement="top"
                          >
                            <Button
                              isIconOnly
