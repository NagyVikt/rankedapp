...
export default function WebshopsPage() {
  ...
  const fetchScreenshotUrl = useCallback(async (shopUrl: string): Promise<string | null> => {
    try {
      const endpoint = process.env.NEXT_PUBLIC_SCREENSHOT_ENDPOINT || '/api/screenshot';
      const response = await fetch(`${endpoint}?url=${encodeURIComponent(shopUrl)}`);
      if (!response.ok) {
        console.error(`Failed to fetch screenshot for ${shopUrl}: ${response.statusText}`);
        return null;
      }
         ...
         .then(results => {
           ...
           setShops(currentShops =>
             currentShops.map(shop => {
               ...
               return shop.isLoadingImage ? { ...shop, isLoadingImage: false } : shop;
             })
           );
         })
         .catch(error => {
