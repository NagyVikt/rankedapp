...
export default function PricingClientComponent({
  ...
  return (
    <div className="flex max-w-4xl flex-col items-center py-24 px-4 sm:px-6 lg:px-8 mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 w-full items-end">
        {' '}
        {/* Added items-end */}
        {plans.map((plan) => (
          <Card
            ...
            className={cn(
              'relative p-3 flex flex-col', // base styles
              {
                ...
                '!border-medium border-default-100 bg-transparent lg:mb-[-3rem]':
                  !plan.mostPopular, // Adjusted vertical alignment
              },
            )}
            shadow="none"
          >
            ...
            <CardHeader className="flex flex-col items-start gap-2 pb-6 pt-6">
              {' '}
              {/* Added pt-6 */}
              <h2
            ...
            <Divider
              className={cn({ 'bg-primary-foreground/20': plan.mostPopular })}
            />{' '}
            {/* Divider color for popular */}
            {/* Use justify-between if CardFooter is not always at bottom */}
            <CardBody className="gap-8 flex-grow">
              {' '}
      ...
      <div className="flex py-2">
        <p className="text-default-400">
          ...
          <Link color="foreground" href="/contact" underline="always">
            {' '}
