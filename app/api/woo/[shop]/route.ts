...
export async function GET(req: NextRequest, { params }: { params: { shop: string } }) {
  ...
  // 1) read shops from cookie
  const shopsCookie = req.cookies.get("my_webshops")?.value || "[]";
  const shops = JSON.parse(decodeURIComponent(shopsCookie)) as { name: string; url: string }[];
  const matched = shops.find((s) => slugify(s.url) === shop);
  if (!matched) return NextResponse.json({ error: "Unknown shop" }, { status: 404 });

  // 2) read Woo creds from cookie
  const credsCookie = req.cookies.get("wooConnections")?.value || "[]";
  const conns = JSON.parse(decodeURIComponent(credsCookie)) as WooEntry[];
  const conn = conns.find((c) => c.selectedShopUrl === matched.url);
  if (!conn) return NextResponse.json({ error: "Missing WooCommerce credentials" }, { status: 401 });

  // 3) normalize + force www
  let raw = matched.url;
  ...
  // 4) fetch Woo on server side with Basic Auth
