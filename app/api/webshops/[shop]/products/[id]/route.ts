...
export async function GET(
  ...
  console.log(
    `[PRODUCT_DETAIL] shopSlug from path: '${shopSlug}', product ID: '${id}'`,
  );
  ...
  try {
    ...
    console.error(
      "[PRODUCT_DETAIL] Error parsing 'my_webshops' cookie:",
      e.message,
    );
  ...
  const matched = shops.find((s) => {
    ...
    console.log(
      `  - slugified s.url ('${s.url}' -> '${base}'): Is '${base === shopSlug}'?`,
    );
    console.log(
      `  - 'www-${base}' ('www-${base}'): Is '${`www-${base}` === shopSlug}'?`,
    );
  ...
  if (!matched) {
    console.error(
      `[PRODUCT_DETAIL] 404 - Unknown shop. No shop found for slug '${shopSlug}' (or 'www-${shopSlug}') in 'my_webshops'.`,
    );
    return NextResponse.json({ error: 'Unknown shop' }, { status: 404 });
  }
  console.log("[PRODUCT_DETAIL] Matched shop from 'my_webshops':", matched);
  console.log(
    `[PRODUCT_DETAIL] Using matched.url for creds lookup: '${matched.url}'`,
  );
  ...
  try {
    ...
    console.error(
      "[PRODUCT_DETAIL] Error parsing 'wooConnections' cookie:",
      e.message,
    );
    return NextResponse.json(
  ...
  if (conns.length > 0) {
    console.log(
      "[PRODUCT_DETAIL] Available selectedShopUrls in 'wooConnections':",
    );
    conns.forEach((c) => console.log(`  - '${c.selectedShopUrl}'`));
  } else {
    console.log(
      "[PRODUCT_DETAIL] 'wooConnections' cookie is empty or contains no entries.",
    );
  }
  ...
  if (!conn) {
    console.error(
      `[PRODUCT_DETAIL] 401 - Missing WooCommerce credentials. No entry in 'wooConnections' with selectedShopUrl === '${matched.url}'.`,
    );
    return NextResponse.json(
  ...
  console.log('[PRODUCT_DETAIL] Found WooCommerce credentials:', {
  ...
  console.log(
    `[PRODUCT_DETAIL] WooCommerce product detail query string: ?${qs.toString()}`,
  );

  // 5) fetch the single product
  const wcRes = await fetch(`${origin}/wp-json/wc/v3/products/${id}?${qs}`, {
    next: { revalidate: 60 },
  });
  console.log(
    `[PRODUCT_DETAIL] WooCommerce product detail API response status: ${wcRes.status}`,
  );

  if (!wcRes.ok) {
    const errorBody = await wcRes.text();
    console.error(
      `[PRODUCT_DETAIL] WooCommerce product detail API error: ${wcRes.status}`,
      errorBody,
    );
    return NextResponse.json(
  ...
  console.log(
    '[PRODUCT_DETAIL] Request successful. Returning product details.',
  );
