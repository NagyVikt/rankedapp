...
export async function GET(
  ...
  console.log(`[PRODUCTS_LIST] page: ${page}, perPage: ${perPage}, sortPrice: ${sortPrice}`);
  ...
  try {
    ...
    console.error("[PRODUCTS_LIST] Error parsing 'my_webshops' cookie:", e.message);
  ...
  const matched = shops.find((s) => {
    ...
    console.log(`[PRODUCTS_LIST] Comparing shopSlug '${shopSlug}' with slugified s.url '${s.url}' -> '${currentSlug}'`);
    return currentSlug === shopSlug;
  });

  if (!matched) {
    console.error(`[PRODUCTS_LIST] 404 - Unknown shop. No shop found with slug '${shopSlug}' in 'my_webshops'.`);
    return NextResponse.json({ error: "Unknown shop" }, { status: 404 });
  }
  console.log("[PRODUCTS_LIST] Matched shop from 'my_webshops':", matched);
  console.log(`[PRODUCTS_LIST] Using matched.url for creds lookup: '${matched.url}'`);
  ...
  try {
    ...
    console.error("[PRODUCTS_LIST] Error parsing 'wooConnections' cookie:", e.message);
  ...
  if (conns.length > 0) {
    console.log("[PRODUCTS_LIST] Available selectedShopUrls in 'wooConnections':");
    conns.forEach(c => console.log(`  - '${c.selectedShopUrl}'`));
  } else {
    console.log("[PRODUCTS_LIST] 'wooConnections' cookie is empty or contains no entries.");
  }

  const conn = conns.find((c) => c.selectedShopUrl === matched.url);

  if (!conn) {
    console.error(`[PRODUCTS_LIST] 401 - Missing WooCommerce credentials. No entry in 'wooConnections' with selectedShopUrl === '${matched.url}'.`);
    return NextResponse.json(
      { error: "Missing WooCommerce credentials for shop URL: " + matched.url },
      { status: 401 }
    );
  }
  console.log("[PRODUCTS_LIST] Found WooCommerce credentials:", { consumerKey: conn.consumerKey ? '***' : undefined }); // Avoid logging secret
  ...
  console.log(`[PRODUCTS_LIST] WooCommerce products query string: ?${qs.toString()}`);

  // 5) fetch that one page of products
  const wcRes = await fetch(`${origin}/wp-json/wc/v3/products?${qs}`, {
    next: { revalidate: 120 }, 
  });
  console.log(`[PRODUCTS_LIST] WooCommerce products API response status: ${wcRes.status}`);

  if (!wcRes.ok) {
    const errorBody = await wcRes.text();
    console.error(`[PRODUCTS_LIST] WooCommerce products API error: ${wcRes.status}`, errorBody);
    return NextResponse.json(
  ...
  console.log(`[PRODUCTS_LIST] Successfully fetched products. Total pages: ${totalPages}`);
  ...
  console.log(`[PRODUCTS_LIST] WooCommerce settings query string: ?${setQs.toString()}`);
  const setRes   = await fetch(
    `${origin}/wp-json/wc/v3/settings/general?${setQs}`,
    { next: { revalidate: 3600 } }
  );
  console.log(`[PRODUCTS_LIST] WooCommerce settings API response status: ${setRes.status}`);
  
  let currency = "USD"; // Default currency
  if (setRes.ok) {
    const settings = await setRes.json();
    const currencySetting = settings.find((s: any) => s.id === "woocommerce_currency")?.value;
    if (currencySetting) {
    ...
    console.warn(`[PRODUCTS_LIST] Failed to fetch WooCommerce settings: ${setRes.status}`, errorBody);
    console.warn(`[PRODUCTS_LIST] Using default currency ${currency} due to settings fetch error.`);
  }


  // 7) return everything together, exposing the total‑pages header
  console.log("[PRODUCTS_LIST] Request successful. Returning products and currency.");
  return NextResponse.json(
    { products, currency },
    {
      headers: {
