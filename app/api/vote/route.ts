...
export async function GET(request: Request) {
  ...
  // }
  console.log(
    `--- Vote GET: User state: ${user ? `Authenticated (${user.id})` : 'Not Authenticated'}`,
  );
  ...
  try {
    ...
    console.error(
      `--- Vote GET: Database error fetching votes for chat ${chatId}:`,
      dbError,
    );
...
export async function PATCH(request: Request) {
  ...
  if (!user) {
    console.error(
      '--- Vote PATCH: No authenticated user found via Supabase SSR.',
    );
    return NextResponse.json(
  ...
  console.log(
    `--- Vote PATCH: User ${currentUserId} attempting to vote on message ${messageId} in chat ${chatId}`,
  );
  ...
  try {
    ...
    console.log(
      `--- Vote PATCH: Vote recorded successfully for user ${currentUserId}.`,
    );
    // Use NextResponse for consistency
    return NextResponse.json({ message: 'Message voted' }, { status: 200 });
  } catch (dbError: any) {
    console.error(
      `--- Vote PATCH: Database error voting for user ${currentUserId}:`,
      dbError,
    );
    return NextResponse.json(
