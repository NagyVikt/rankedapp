...
import { convertToCoreMessages, createDataStreamResponse, streamText, type CoreMessage } from 'ai'; // Import convertToCoreMessages directly, and CoreMessage type
import { z } from 'zod';
...
type StoredMessage = Omit<AiMessage, 'role' | 'content'> & { // Omit AiMessage's role and content to redefine them
  chatId: string;
...
export async function POST(request: Request) {
  ...
  const { id: chatId, messages, modelId, reasoningModelId, experimental_deepResearch = false }: {
    id: string;
  ...
  try {
      ...
      if (!chat) {
        console.log(`--- Chat POST: Creating new chat ${chatId} for user ID: ${currentUserId} ---`);
        // generateTitleFromUserMessage expects CoreMessage, which userMessage is.
        const title = await generateTitleFromUserMessage({ message: userMessage });
        await saveChat({ id: chatId, userId: currentUserId, title, visibility: 'private' });
      } else {
        console.log(`--- Chat POST: Checking ownership for existing chat ${chatId}. Owner: ${chat.userId}, Current User: ${currentUserId} ---`);
        if (chat.userId !== currentUserId) {
          console.error(`User ${currentUserId} attempted to post to chat ${chatId} owned by ${chat.userId}`);
      ...
      console.error(`--- Chat POST: Database error during chat handling for chat ${chatId}:`, dbError);
  ...
  try {
      ...
      console.log(`--- Chat POST: Saved user message ${userMessageId} for chat ${chatId}`);
      ...
      return createDataStreamResponse({
          ...
          const result = streamText({
            ...
            tools: {
               search: {
                    ...
                    execute: async ({ query, maxResults = 5 }) => { /* ... Firecrawl search logic ... */ return `Search results for: ${query}`; }
               },
               extract: {
                    description: "Extract structured data from web pages.",
                    parameters: z.object({ urls: z.array(z.string().url()), prompt: z.string() }),
                    execute: async ({ urls, prompt }) => { /* ... Firecrawl extract logic ... */ return `Extracted data from ${urls.join(', ')}`; }
               },
               scrape: {
                    description: "Scrape raw content from a single URL.",
                    parameters: z.object({ url: z.string().url() }),
                    execute: async ({ url }) => { /* ... Firecrawl scrape logic ... */ return `Scraped content from ${url}`; }
               },
               deepResearch: {
                    ...
                    execute: async ({ topic: initialTopic, maxDepth = 7 }) => { /* ... Deep research logic ... */ return `Deep research summary for ${initialTopic}`; }
               },
            },
            onFinish: async ({ text, toolCalls, toolResults, finishReason, usage }) => {
                console.log(`--- Chat POST: AI stream finished for chat ${chatId}. Reason: ${finishReason} ---`);
                ...
                if (toolCalls && toolCalls.length > 0) {
                    toolCalls.forEach(toolCall => {
                        finalMessagesToSave.push({
                            ...
                            content: JSON.stringify([{ // Content is stringified tool call structure
                                type: 'tool-call',
                ...
                if (toolResults && toolResults.length > 0) {
                    toolResults.forEach(toolResult => {
                        finalMessagesToSave.push({
                            ...
                            content: JSON.stringify([{ // Content is stringified tool result structure
                                type: 'tool-result',
                ...
                if (finalMessagesToSave.length > 0) {
                    try {
                        ...
                        console.log(`--- Chat POST: Saved ${finalMessagesToSave.length} assistant/tool messages for chat ${chatId}.`);
                    }
                    catch (dbSaveError) {
                        console.error(`--- Chat POST: Failed to save assistant/tool messages for chat ${chatId}:`, dbSaveError);
                    }
                }
            },
       ...
       console.error(`--- Chat POST: Database error saving user message for chat ${chatId}:`, dbError);
...
export async function DELETE(request: Request) {
    ...
    try {
        ...
        return new Response(`An error occurred: ${error.message || 'Unknown error'}`, { status: 500 });
    }
}
