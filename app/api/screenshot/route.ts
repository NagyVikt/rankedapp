...
console.log("SUPABASE_SERVICE_ROLE_KEY:", supabaseServiceKey ? 'Loaded (length: ' + (supabaseServiceKey?.length ?? 0) + ')' : 'MISSING!');
console.log("-------------------------");

// Validate Supabase config at module load (optional, good for early feedback)
if (!supabaseUrl || !supabaseServiceKey) {
    console.error("CRITICAL: Supabase URL or Service Key environment variable is missing at module load! Ensure .env.local is correct and server was restarted.");
    ...
    if (!supabase) {
        ...
        if (su_url && su_key) {
            try {
             ...
             console.error("Attempted to initialize Supabase client, but NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY env vars are missing!");
             return null;
        }
    }
    return supabase;
...
export async function GET(request: Request) {
    console.log(`[Request Start] URL: ${request.url}. Service Key ${process.env.SUPABASE_SERVICE_ROLE_KEY ? 'seems set' : 'is NOT SET (check env vars)'}.`);

    const supabaseClient = getSupabaseClient();
    if (!supabaseClient) {
         console.error("FATAL: Supabase client is not available. This usually means environment variables for Supabase are missing or incorrect. Check server logs for initialization errors.");
         return NextResponse.json({ error: 'Server configuration error: Supabase client could not be initialized.' }, { status: 500 });
    }
    ...
    try {
        ...
        console.log(`Processing URL: "${urlToScreenshot}", Target Filename: "${filename}"`);

        // --- 2. Check Supabase Cache ---
        console.log(`Checking cache for "${filename}" in bucket "${BUCKET_NAME}"...`);
        ...
        try {
            ...
            const { data: publicUrlDataResult } = supabaseClient
                .storage
                .from(BUCKET_NAME)
            ...
            if (publicUrlDataResult?.publicUrl && !publicUrlDataResult.publicUrl.includes('/render/path')) {
                 console.log(`Potentially valid public URL from getPublicUrl(): "${publicUrlDataResult.publicUrl}". Verifying actual file existence with list().`);
                 ...
                 if (listError) {
                      ...
                      console.error(`Supabase cache check error (during list operation for "${filename}"):`, listError.message);
                      if (listError.message.includes('signature') || listError.message.includes('authorization') || listError.message.includes('authenticated')) {
                           throw new Error(`Supabase authentication/authorization error listing file: ${listError.message}. VERIFY SUPABASE SERVICE KEY AND BUCKET POLICIES.`);
                      }
                      ...
                      console.warn("Proceeding to generate screenshot due to a non-auth error during cache list operation or if file not found by list.");
                 } else if (fileListData && fileListData.length > 0) {
                      // File exists according to list()
                      console.log(`Cache hit confirmed by list() for "${filename}". Using public URL: ${publicUrlDataResult.publicUrl}`);
                      publicUrl = publicUrlDataResult.publicUrl;
                 } else {
                      // File does not exist according to list()
                      console.log(`Cache miss for "${filename}" (file not found by list() operation).`);
                 }
            } else {
                // This case means getPublicUrl() returned a null/empty/placeholder publicUrl, or publicUrlDataResult was null.
                console.log(`Initial getPublicUrl() for "${filename}" did not yield a usable public URL (URL was: ${publicUrlDataResult?.publicUrl || 'null/undefined'}). Treating as cache miss.`);
            }
             ...
             console.error(`Error during Supabase cache check phase for "${filename}":`, cacheCheckPhaseError.message);
             if (cacheCheckPhaseError.message.includes('Supabase authentication') || cacheCheckPhaseError.message.includes('authorization')) {
             ...
             console.warn("Proceeding to generate screenshot due to an error encountered in the cache check phase.");
        }

        if (publicUrl) {
             console.log(`[Cache Hit] Returning cached URL for "${filename}": ${publicUrl}`);
             return NextResponse.json({ imageUrl: publicUrl });
        } else {
             console.log(`[Cache Miss] Proceeding to generate screenshot for "${filename}".`);
        }
        ...
        const navigationResponse = await page.goto(urlToScreenshot, { waitUntil: 'networkidle0', timeout: TIMEOUT_MS });
        ...
        if (navigationResponse && !navigationResponse.ok()) {
            ...
            console.error(`Navigation failed with HTTP status: ${status} for URL: ${urlToScreenshot}`);
            throw new Error(`Page load failed with status ${status}. URL: ${urlToScreenshot}`);
        }

        // --- 5. Take Screenshot ---
        const screenshotDelayMs = 2000;
        console.log(`Waiting ${screenshotDelayMs}ms for potential dynamic content rendering...`);
        ...
        console.log("Screenshot captured successfully (buffer size: " + screenshotBuffer.length + " bytes).");
        ...
        if (uploadError) {
            console.error("Supabase upload error:", uploadError);
             if (uploadError.message.includes('signature') || uploadError.message.includes('authorization') || uploadError.message.includes('authenticated')) {
                 throw new Error(`Supabase authentication/authorization error uploading file: ${uploadError.message}. VERIFY SUPABASE SERVICE KEY AND BUCKET POLICIES (e.g. allow authenticated uploads).`);
            }
        ...
        const { data: finalUrlObject } = supabaseClient // Again, getPublicUrl is sync and returns { data: { publicUrl: string } }
            .storage
            .from(BUCKET_NAME)
            .getPublicUrl(filename); // filename should be same as uploadData.path if no transformation

        if (!finalUrlObject?.publicUrl) {
             console.error("Public URL was null/empty after upload and second getPublicUrl call for:", filename);
             throw new Error("Failed to retrieve public URL from Supabase after successful upload. The final URL object or its publicUrl property was unexpectedly null.");
        }

        console.log(`[After Upload] Returning newly generated URL: ${finalUrlObject.publicUrl}`);
        return NextResponse.json({ imageUrl: finalUrlObject.publicUrl });

    } catch (error: any) {
        console.error("[Operation Error] An error occurred in the GET handler:", error); // Log the full error object
        // console.error("[Operation Error] Error message:", error.message);
        // console.error("[Operation Error] Error stack:", error.stack);


        const errorMessage = error.message.includes('Supabase authentication') || error.message.includes('authorization')
        ...
        if (page && !page.isClosed()) {
            ...
            } catch (e: any) { console.error("Error closing Puppeteer page:", e.message); }
        } else if (page?.isClosed()){
             console.log("Puppeteer page cleanup skipped (already closed).");
        } else {
             console.log("Puppeteer page cleanup skipped (page was not initialized or already null).");
        }
        ...
        if (browser) {
            ...
            } catch (e: any) { console.error("Error closing Puppeteer browser:", e.message); }
        } else {
             console.log("Puppeteer browser cleanup skipped (browser was not initialized, already null, or launch failed).");
        }
