...
export async function GET(request: Request) {
  ...
  if (!user) {
    console.log(
      '--- History GET: No authenticated user found via Supabase SSR.',
    );
  ...
  console.log(
    `--- History GET: Found authenticated Supabase user ID: ${currentUserId}`,
  );
  ...
  try {
    ...
    console.log(
      `--- History GET: Found ${chats.length} chats for user ${currentUserId}.`,
    );
    return NextResponse.json(chats, { status: 200 });
  } catch (dbError: any) {
    console.error(
      `--- History GET: Database error fetching chats for user ${currentUserId}:`,
      dbError,
    );
    return NextResponse.json(
